...
namespace fwdpp_cython {
		     ...
		     const unsigned & seed)
  {
    ...
    double mu = theta/(4.*double(N)),littler=rho/(4.*double(rho));
    GSLrng_t<GSL_RNG_MT19937> r(seed);
	...
	double wbar = sample_diploid(r.get(),
					    ...
					    std::bind(infsites(),r.get(),&pop.mut_lookup,generation,
					    ...
					    std::bind(mutation_remover(),std::placeholders::_1,0,2*N));
	remove_fixed_lost(&pop.mutations,&pop.fixations,&pop.fixation_times,&pop.mut_lookup,generation,2*N);
